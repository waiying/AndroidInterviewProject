...
public class SinglePlayerButton extends PennyPopButtons{
	 ...
	 */
	public SinglePlayerButton(FileHandle file, Screen settingsScreen, SpriteBatch sb) {
		super(file);
	...
	public void addClickEvent(ImageButton button) {
		InputListener listener = new InputListener(){
			public boolean touchDown(InputEvent event, float x, float y, int pointer, int button){
				// Switch to Connect 4 GameScreen when 1 Player button is clicked
				settingsScreen.hide();
				settingsScreen.dispose();
				
				gameScreen = new GameScreen(sb, true); // second arg is true to turn on AI mode
				gameScreen.show();
